...
def MultiBoxTarget(anchor, label):
    ...
    Returns:
        ...
        背景类别设为0，并令从零开始的目标类别的整数索引自加1（1为狗，2为猫）。
    """
    ...
    def MultiBoxTarget_one(anc, lab, eps=1e-6):
        ...
        """
        an = anc.shape[0]
        assigned_idx = assign_anchor(lab[:, 1:], anc)  # (锚框总数, ) assign_anchor得到锚框的索引
        # print(assigned_idx)
        # print((assigned_idx >= 0).float())
        # bbox_mask的内容是被记录的锚框，有用的就该位置大于0，且总数为shape[0]
        ...
        center_assigned_bb = xy_to_cxcy(assigned_bb)  # 真实锚框
    ...
    batch_cls_labels = []
    for b in range(bn):
        # offset是偏移量 bbox_mask返回掩码，即0为背景，cls_labels是选中的锚框的类别
